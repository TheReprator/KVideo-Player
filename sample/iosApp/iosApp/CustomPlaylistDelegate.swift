//
// Created by Deep Sandhya on 18/08/25.
//
...
func toCustomUrl(_ url: String) -> String {
    return customPlaylistScheme + String(url.dropFirst(5).dropLast(3)) + hlsExt
}

func toOriginalUrl(_ url: String) -> String {
    return httpsScheme + String(url.dropFirst(4).dropLast(4)) + dashExt
}
...
private extension CustomPlaylistDelegate {
    ...
    func extractMpdInfo(_ textToDec: String) -> MpdInfo {
        ...
        let range = NSRange(textToDec.startIndex..<textToDec.endIndex, in: textToDec)

        var mpdInfo = MpdInfo(key: [], item: [])

        for match in regex.matches(in: textToDec, range: range) {
            if let keyRange = Range(match.range(at: 1), in: textToDec),
               let itemRange = Range(match.range(at: 2), in: textToDec) {
                mpdInfo.key.append(String(textToDec[keyRange]))
                mpdInfo.item.append(String(textToDec[itemRange]))
            }
        }
        return mpdInfo
    }
    ...
    func getDuration(_ rawDuration: String) throws -> Float {
        ...
        let timePatternFull = try! Regex<(Substring, Substring, Substring)>(#"(?:([0-9]+)H)?(?:([0-9]+)M)?(?:([0-9]+)S)?"#)
        let timePatternSec  = try! Regex<(Substring)>(#"(?:([0-9]+)S)"#)

        // Case: Hours, Minutes, Seconds
        if let match = try? timePatternFull.firstMatch(in: rawDuration) {
            let (hour, minute, second) = match.output

            let h = Float(hour) ?? 0
            let m = Float(minute) ?? 0
            let s = Float(second) ?? 0
            return h * 3600 + m * 60 + s
        }

        // Case: Only Seconds
        if let match = try? timePatternSec.firstMatch(in: rawDuration) {
    ...
    func createMediaPlaylist(_ mpdInfo: MpdInfo, url: String) throws -> [String] {
        ...
        while let idx = curIdx {
            ...
            if segmentType.contains(videoKey) {
                ...
                output = maxSegmentDuration + startSequence + playlistType + mapInit + segmentUnit + video_info + info
                ...
                let maxSegmentDuration = "#EXT-X-TARGETDURATION:\(maxDuration)\n"
                output = maxSegmentDuration + startSequence + playlistType + mapInit + segmentUnit + audio_info + channel_info + info
            }
